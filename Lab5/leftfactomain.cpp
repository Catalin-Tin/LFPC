...
{
	int n,p,contor=0;
	vector<string> prodleft(n),prodright(n);
    cout<<"HOW MANY RULES ARE THERE ? ";
	cin>>n;
    cout<<"\nEnter number of non terminals: ";//Recursion
    cin>>p;
    ...
    for(i=0;i<nonter.size();++i) {
        ...
        for(j=0;j<prod[i].size();++j) {
            contor++;
            cout<<"\n"<<nonter[i]<<" -> "<<prod[i][j];
         prodleft[contor]=nonter[i];
          prodright[contor]=prod[i][j];
        }
    }
    int h;
    cout<<endl;
    ...
    for(i=0;i<n;++i) {
        for(l=i+1;l<n;++l) {
            if(prodleft[l]==prodleft[i]) {
                int k=0;
                string com="";
                while(k<prodright[i].length()&&k<prodright[l].length()&&prodright[i][k]==prodright[l][k]) {
                    com+=prodright[i][k];
                    ++k;
                }
                if(k==0)
                ...
                int o;
                for(o=l+1;o<n;++o) {
                    if(comleft==prodleft[o]&&com==prodright[o].substr(0,fmin(k,prodright[o].length()))) {
                        prodleft[o]+=string(itoa(e,buffer,10));
                        prodright[o]=prodright[o].substr(k,prodright[o].length()-k);
                    }
                }
