...
{
	int p,counter=0;
	long long int i,j,k,l,n=7,m=9999999999,mini,ma=0;
    string s,st,ch,sc="",result,fs,maxi,rs="";
    vector<string>sl;
    vector<string>sp;
	string strin,f[100];
    ...
    for(i=0;i<nonter.size();++i)
        cout<<nonter[i]<<" ";
    cout<<"\n\nGRAMMAR AFTER LEFT RECURSION: "<<endl;
    for(i=0;i<nonter.size();++i) {
        int j;
        for(j=0;j<prod[i].size();++j) {
            counter++;
          cout<<"\n"<<nonter[i]<<" -> "<<prod[i][j];
            //strin+=nonter[i]+"="+prod[i][j]+"\n";
            //set_map(strin);
            f[counter]=nonter[i]+"="+prod[i][j]+"\n";
        }
    }
    ...
    cout<<endl;
    cout<<"GRAMMER AFTER LEFT FACTORING"<<endl;
